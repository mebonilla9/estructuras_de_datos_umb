 ...
 */
package co.appreactor.ordenamiento.persistencia.orden.ordenes;
 ...
 * comparison sort.It generalizes an exchanging sort, such as insertion or
 ...
 elements that are far apart before finishing with neighboring elements. Starting with far apart elements can move some out-of-place elements into
...
public class ShellSort<T extends Comparable<T>> {
  ...
  public static <T extends Comparable<T>> T[] sort(int[] shells, T[] unsorted) {
    for (int gap : shells) {
      // Allocate arrays
      List<List<T>> subarrays = new ArrayList<>(gap);
      for (int i = 0; i < gap; i++) {
